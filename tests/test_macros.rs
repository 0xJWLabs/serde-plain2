...
impl From<serde_plain2::Error> for Test2Error {
    fn from(_: serde_plain2::Error) -> Test2Error {
        Test2Error
    }
}
...
fn test_derive_deserialize() {
    let test: TestStruct = serde_plain2::from_str("42").unwrap();
    assert_eq!(TestStruct(42).0, test.0);
}

#[test]
fn test_derive_serialize() {
    let test = serde_plain2::to_string(&TestStruct(42)).unwrap();
